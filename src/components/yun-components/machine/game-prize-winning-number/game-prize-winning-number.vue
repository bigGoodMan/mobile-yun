...
<template>
  ...
  >
    <div class="game-prize-winning-number">
      <h5 class="size-30 color-3 padding-20-30 game-prize-winning-number-title">
        获奖局数
      </h5>
      <div class="padding-20-30">
        ...
        <p class="size-24 color-9 margin-0 game-prize-winning-number-tip">
          毛利=收入*毛利率，即若收入100元，期望利润为80元则 毛利率设80%
        </p>
      </div>
      <CancelConfirmBtn
        :loading="loading"
...
export default {
  ...
  methods: {
    handleProfitInput: debounce({
      callback: function(event) {
        const val = event.target.value;
        this.profit = val;
        const awardValue = Math.ceil(
          this.moneyCost / (this.coinsSell * this.coinsValue * (1 - val / 100))
        );
        ...
        if (
          !positiveIntegerRegularTool(this.awardValue) ||
          this.awardValue <= 0 ||
          this.awardValue > 99
        ) {
    ...
    handleAwardInput: debounce({
      callback: function(event) {
        const val = event.target.value;
        this.awardValue = val;
        const profit =
          ((1 - this.moneyCost / val / this.coinsSell / this.coinsValue) *
            100) |
          0;
        this.profit = profit;
        if (!positiveIntegerRegularTool(val) || val <= 0 || val >= 100) {
          this.validatorContent =
            "获奖局数超出限制（1~99整数），请修改获奖局数";
          return;
        }
        if (
          !positiveIntegerRegularTool(profit) ||
          profit < 0 ||
          profit >= 100
        ) {
