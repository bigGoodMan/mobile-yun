...
<template>
  <ul class="stock-taking-list bgcolor-f2">
    <template v-for="items of list">
      <li
        :key="items.inventory_id"
        class="stock-taking-list-cell bgcolor-f"
      ...
      >
        <div class="flex-row flex-between-center stock-taking-list-first">
          ...
          <span :class="['size-24', getColor(items.status)]">{{
            status[items.status]
          }}</span>
        </div>
        ...
        <div
          class="stock-taking-list-third flex-row flex-between-center size-24 color-393d49"
        >
          <span>
            盘盈<i class="color-success">{{ items.profit_num }}</i
            >个，盘亏<i class="color-error">{{ items.deficit_num }}</i
            >个
          </span>
