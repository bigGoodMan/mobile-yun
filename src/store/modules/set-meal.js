...
export default {
  ...
  mutations: {
    ...
    SET_MEAL_GETRECHARGEPACKAGELIST_MUTATION(state, { list, id }) {
      state.rechargePackageList = list.map(v => {
        const packages = v.package
          .map((its, inx) => {
            const coins = its["coins" + (inx + 1)] - 0;
            const coin = coins ? coins + "币" : "";
            const discount = coins
              ? (((v.money / coins) * 100) | 0) / 10 + "折"
              : "";
    ...
    // 设置一条体验券
    SET_MEAL_SETEXPERIENCETICKETITEM_MUTATION() {}
  },
  actions: {
    // 获取充值套餐列表
    async SET_MEAL_GETRECHARGEPACKAGELIST_ACTION({ commit }, storeId) {
      ...
      if (res.return_code === "0") {
        commit("SET_MEAL_GETRECHARGEPACKAGELIST_MUTATION", {
          list: res.data.list,
          id: res.data.id
        });
      }
    ...
    // 保存充值套餐
    async SET_MEAL_SAVERECHARGEPACKAGELIST_ACTION(
      { commit, state },
      { storeId, list, data }
    ) {
      let id;
      ...
      if (res.return_code === "0") {
        commit("SET_MEAL_SAVERECHARGEPACKAGELIST_MUTATION", {
          list,
          id: res.data.id
        });
      }
