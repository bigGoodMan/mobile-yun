...
export default {
  ...
  actions: {
    ...
    async MACHINE_MACHINEDETAIL_ACTION({ commit }, { id }) {
      const res = await getMachineDetailApi({ machine_id: id });
    ...
    // 保存预警参数
    async MACHINE_SAVEEARLYPARAM_ACTION(
      { commit },
      { machineId, canClawNum, canClawSafeNum }
    ) {
    ...
    // 保存游玩单价
    async MACHINE_SAVEPLAYPRICE_ACTION(
      { commit },
      { machineId, coinsSell, coinSellVip }
    ) {
    ...
    // 下发运营参数
    async MACHINE_SETOPERATEPARAM_ACTION(
      { commit, state },
      { machineId, data }
    ) {
      data = {
