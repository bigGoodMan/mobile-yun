...
<template>
  <div>
    ...
    <div class="margin-top-20">
      <h5 class="margin-0 padding-20-30 bgcolor-f size-0 flex-row flex-between-center">
        <div>
          <span class="size-28">礼品列表</span>
          <span class="size-24 color-7 padding-left-10">共{{typeNum}}款，{{total.num}}个，{{total.money_cost}}元</span>
        </div>
      ...
      <div class="margin-bottom-20" v-for="items of giftStorageList" :key="items.gift_id">
        <GiftStorageItem :result="items" @trigger-delete="GIFT_DELETEGIFTSTORAGELIST_MUTATE(items)"/>
      </div>
    </div>
    <!-- 按钮 -->
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      for (let i = 0; i < this.giftStorageList.length; ++i) {
        ...
        if (!(/^[0-9]+(\.[0-9]{1,2})?$/.test(items.money_cost) && items.money_cost >= 0.01 && items.money_cost <= 999)) {
          this.$Tip.warning('成本单价数据不正确')
          return
        }
        if (!(/^[0-9]+$/.test(items.num) && items.num >= 1 && items.num <= 99999)) {
