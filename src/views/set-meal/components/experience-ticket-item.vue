  ...
  <dl class="experience-ticket-item">
    <dt class="title flex-row flex-between-stretch">
      <div class="flex-column flex-center-start">
        ...
        <span class="size-24 color-9">{{
          getDateInterval(itemData.start_time, itemData.end_time)
        }}</span>
      </div>
    ...
    <dd
      v-for="(items, index) of itemData.card_info"
      :key="index"
      class="content"
    >
      <div class="flex-row flex-between-center size-26 padding-30-0">
        ...
        <span class="color-9">{{
          getDateInterval(items.start, items.end)
        }}</span>
        <span
          >数量<i class="padding-left-10 no-style">{{ items.num }}</i></span
        >
      </div>
      ...
      <ul class="no-ul flex-row flex-between-center">
        ...
        <li>
          <van-button
            ...
            radius="0.1rem"
            :disabled="itemData.status !== '0'"
            @click="handleTakeEffect"
            >{{ itemData.status === "0" ? "生效" : "已生效" }}</van-button
          >
        </li>
        <li>
          <van-button
            ...
            size="small"
            radius="0.1rem"
            :disabled="itemData.status !== '1'"
            @click="handleLink(itemData.url)"
            >链接</van-button
          >
        </li>
        <li>
          <van-button
            ...
            >{{ itemData.status === "2" ? "已停用" : "停用" }}</van-button
          >
        </li>
      </ul>
    </dd>
    <dd ref="selectRef" style="display: none;">{{ itemData.url }}</dd>
    <dd>
      <van-popup v-model="show"
        ><div class="size-24 padding-20-30">{{ itemData.url }}</div></van-popup
      >
    </dd>
  </dl>
</template>
...
export default {
  ...
  methods: {
    ...
    getDateInterval(start, end) {
      return `${moment(start * 1000).format("YYYY-MM-DD")}~${moment(
        end * 1000
      ).format("YYYY-MM-DD")}`;
    },
    handleLink(url) {
