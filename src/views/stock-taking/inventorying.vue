...
<template>
  <div class="inventorying">
    ...
    <!-- 统计&添加礼品 -->
    <div
      class="flex-row flex-between-center bgcolor-f inventorying-header-data"
    >
      <div class="flex-column flex-between-start">
        <span class="size-28 color-393d49 inventorying-store">{{
          inventoryingStoreName
        }}</span>
        <span class="size-24 color-393d49"
          >共<i class="color-success">{{ typeNum }}</i
          >款，实盘数<i class="color-success">{{ inventoryNum }}</i></span
        >
      </div>
      <HhfButton
        type="info"
    ...
    <div class="inventorying-btn">
      <ul
        class="inventorying-btn-container flex-row flex-center fixed-max-width bottom-0"
      >
        <li style="width: 30%;">
          <HhfButton
            type="default"
...
export default {
  ...
  methods: {
    ...
    handleSearchInput(val) {
      ...
      this.timer = setTimeout(() => {
        if (val === "") {
          ...
          this.giftList = this.inventoryGiftList.filter(
    ...
    handleSubmit() {
      ...
      for (const items of $this.inventoryGiftList) {
        if (items.num === "") {
          this.$Tip.warning({
            message: "有实盘数未填"
          });
