...
<template>
  <div class="time-switch">
    ...
    <div class="container">
      ...
      <div
        v-if="machineValue.machine_id"
        class="bgcolor-f content flex-row flex-between-center padding-20-30"
      >
        <div
          class="img-container"
          @click.stop="
            APP_IMAGEPREVIEW_MUTATE({
              previewImage: [machineValue.img || ''],
              imagePreviewShow: true
            })
          "
        >
          <img v-lazy="machineValue.img || ''" class="img" />
        </div>
        <ul class="no-ul flex-1 size-28">
          <li class="flex-row flex-between-center list">
            <span>型号：</span
            ><span
              >{{ machineValue.machine_type_name }}-{{ machineValue.no }}</span
            >
          </li>
          <li class="flex-row flex-between-center list">
            <span>开机：</span
            ><span :class="machineValue.turn_on_time ? 'color-info' : ''">{{
              machineValue.turn_on_time || "未设置"
            }}</span>
          </li>
          <li class="flex-row flex-between-center list">
            <span>关机：</span
            ><span :class="machineValue.turn_off_time ? 'color-info' : ''">{{
              machineValue.turn_off_time || "未设置"
            }}</span>
          </li>
        </ul>
      </div>
        ...
        <dl class="padding-20-30 size-28">
          ...
          <dd class="size-36 padding-bottom-20 flex-row flex-between-center">
            ...
            <div class="flex-row flex-between-center">
              <input
                type="tel"
                :class="['time-input', timeError[0] ? 'border-error' : null]"
                maxlength="2"
                :value="openHour"
                @input="handleInput($event.target, 'openHour')"
              />
              :
              <input
                type="tel"
                :class="['time-input', timeError[1] ? 'border-error' : null]"
                maxlength="2"
                :value="openMinute"
                @input="handleInput($event.target, 'openMinute')"
              />
            </div>
            <van-button type="info" size="small" @click="handleSave('open')"
              >保存</van-button
            >
          </dd>
          ...
          <dd class="size-36 flex-row flex-between-center">
            ...
            <div class="flex-row flex-between-center">
              <input
                type="tel"
                :class="['time-input', timeError[2] ? 'border-error' : null]"
                maxlength="2"
                :value="closeHour"
                @input="handleInput($event.target, 'closeHour')"
              />
              :
              <input
                type="tel"
                :class="['time-input', timeError[3] ? 'border-error' : null]"
                maxlength="2"
                :value="closeMinute"
                @input="handleInput($event.target, 'closeMinute')"
              />
            </div>
            <van-button type="info" size="small" @click="handleSave('close')"
              >保存</van-button
            >
          </dd>
        </dl>
...
export default {
  ...
  computed: {
    ...
    machineValue: {
      get() {
        const arr = this.machineInfoColumns.filter(
          item => item.machine_id === this.machineId
        );
        return arr.length > 0 ? arr[0] : {};
      }
    },
  ...
  methods: {
    ...
    handleConfirm(data) {
      ...
      getSwitchgearDetailApi({ store_id: this.storeId }).then(res => {
        ...
        if (res.return_code === "0") {
          this.areaColumns = res.data.area.map(item => {
            return {
              text: item.name,
              ...item
            };
          });
          this.machineColumns = res.data.machine_list.map(item => {
            return {
              text: item.no,
              ...item
            };
    ...
    handleInput: debounce({
      callback(target, type) {
        ...
        switch (type) {
          case "openHour":
            if (
              (!positiveIntegerRegularTool(val) && val) ||
              val < 0 ||
              val >= 24
            ) {
    ...
    handleSave() {
      ...
      const { machineId, storeId, areaId, machineColumns } = this;
        ...
        if (res.return_code === "0") {
          this.machineColumns = machineColumns.map(v => {
            if (v.machine_id === machineId) {
              return { ...v, turn_on_time: onTime, turn_off_time: offTime };
            }
            return v;
          });
