  ...
  <div class="firmware-update">
    ...
    <div class="main">
      <div class="container bgcolor-f2">
        ...
        >
          <div class="content size-28 bgcolor-f">
            <h5
              class="margin-0 padding-20-30 bgcolor-f size-28 flex-row flex-between-center"
            >
              <span>我的机台</span
              ><span
                v-if="awaitUpdateColumns.length > 0"
                class="color-info"
                @click="putEquipmentUpdate"
                >一键更新</span
              >
            </h5>
            <div class="border"></div>
            ...
            >
              <template v-if="items.items.length > 0">
                <li v-for="its of items.items" :key="its.machine_id">
                  <TabulatingCard :img="its.img">
                    <template v-if="its.upgrade_status === '2'" v-slot:tag
                      >待更新</template
                    >
                    <template v-slot:title
                      >{{ its.name }}--{{ its.no }}</template
                    >
    ...
    <div v-if="status">
      <EquipmentUpdateProgress
        :status="status"
        :pivot-text="progressNum"
        :percentage="percentage"
        @fail-canel="handleClose"
        @fail-retry="handleRetry"
        @success-confirm="handleClose"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    awaitUpdateColumns() {
      ...
      this.columns.forEach(v => {
        const itemsArr = v.items.filter(item => {
    ...
    awaitMidsParams() {
      return this.awaitUpdateColumns.map(v => ({
        mid: v.machine_id,
        task_id: v.task_id
      }));
    }
  ...
  methods: {
    ...
    async judgeEquipmentUpdateStatus() {
      ...
      const [err, res] = await errorCaptured(judgeEquipmentUpdateStatusApi, {
      ...
      if (res.return_code === "0") {
        const status = res.data; // 0 无需更新 1是更新中
        if (status === "1") {
    ...
    async getEquipmentUpdateProgress() {
      clearTimeout(this.timer);
      this.timer = null;
      const [err, res] = await errorCaptured(getEquipmentUpdateProgressApi, {
        store_id: this.store_id,
    ...
    async getEquipmentUpdateList() {
      ...
      const [err, res] = await errorCaptured(getEquipmentUpdateListApi, {
        store_id: this.store_id
      });
      this.$toast.clear();
      if (err) {
    ...
    handleRefresh() {
      this.getEquipmentUpdateList().then(() => {
